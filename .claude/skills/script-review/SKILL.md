...
- **Check research asset usage** (see Research Asset Validation below)

## Script Structure Validation

Every shot must contain:
- **画面类型**: One of `remotion`, `实景拍摄`, `固定图片`, `ai生图`, `ai生视频`, `ai背景图`
- **画面**: Specific visual direction
- **口播**: Voiceover text
- **生图提示词**: Required only when 画面类型 = `ai生图`
- **生视频提示词**: Required only when 画面类型 = `ai生视频`
- **背景图提示词**: Required only when 画面类型 = `ai背景图`（英文 prompt，不含文字）
...
When reviewing a script that has `assets/research/` collected by the research phase, check how well the script leverages the collected materials.

### Step 1: Read Available Assets

1. Check if `assets/research/manifest.json` exists in the project directory
2. If it exists, read it to get the list of stock and reference assets with their tags/metadata
3. Read the `## 视觉素材清单` section from `research.md` for a quick overview
4. If no assets were collected, skip this validation section entirely

### Step 2: Validate Asset Usage

**Check for:**

| Issue | Severity | Description |
|-------|----------|-------------|
| Stock asset unused | Minor | A stock image exists with relevant tags but the script uses `ai生图` for a similar shot instead |
| Reference asset ignored | Info | Reference images exist but the script's `画面` descriptions don't reflect the visual information available |
| Wrong asset type | Major | Script marks a `reference/` asset as `固定图片` (reference-only license, not for production use) |

### Step 3: Report in Review

Add an asset usage section to the review output:

```
## Research Asset Validation 📸
- 收集素材: X reference + Y stock
- 脚本引用: Z 处使用 stock 素材作为固定图片
- 建议: [如有可优化的镜头，列出具体建议]
```

**Auto-fix**: If a stock asset clearly matches a shot that uses `ai生图`, suggest changing the `画面类型` to `固定图片` with the stock asset filename. Apply after user confirmation.
